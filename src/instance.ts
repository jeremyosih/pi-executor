...
import {
	...
	DEFAULT_REACHABILITY_POLL_INTERVAL_MS,
	GLOBAL_CONFIG_FILE_NAME,
	INSTANCE_DIRECTORY_NAME,
	INSTANCE_LOCAL_DATA_DIRECTORY,
	INSTANCE_LOG_FILE_NAME,
	INSTANCE_PID_FILE_NAME,
	INSTANCE_RECORD_FILE_NAME,
} from "./constants.js";
...
type EnsureExecutorRunningOptions = ResolveExecutorInstanceOptions &
	ReachabilityOptions & {
		spawnImpl?: SpawnFunction;
	};

const DEFAULT_FETCH: FetchLike = fetch;
const DEFAULT_SPAWN: SpawnFunction = (command, args, options) => spawn(command, args, options);
...
const isValidPort = (value: number): boolean => Number.isInteger(value) && value >= 1 && value <= 65_535;
...
const deriveInstance = (cwdRealpath: string, port: number, agentDir: string): ExecutorInstance => {
...
const readInstanceRecord = async (filePath: string): Promise<ExecutorInstance | null> => {
	...
	try {
		...
		const parsed = JSON.parse(text) as Partial<ExecutorInstance>;
		if (
...
const writeInstanceRecord = async (instance: ExecutorInstance, agentDir: string): Promise<void> => {
...
export const resolveExecutorInstance = async (
	cwd: string,
	config: ResolvedPiExecutorConfig,
	options?: ResolveExecutorInstanceOptions,
...
export const isReachable = async (baseUrl: string, options?: ReachabilityOptions): Promise<boolean> => {
	try {
...
export const waitForReachability = async (
	baseUrl: string,
	expected: boolean,
	timeoutMs: number,
...
export const startExecutorWebDetached = async (
	cwd: string,
...
export const ensureExecutorRunning = async (
	cwd: string,
	config: ResolvedPiExecutorConfig,
	options?: EnsureExecutorRunningOptions,
): Promise<ExecutorInstance> => {
	...
	if (!(await availability(instance.port))) {
		throw new Error(
			`Port ${instance.port} is already in use by another process. Refusing to reuse a random reachable executor for ${instance.cwdRealpath}.`,
		);
	}
	...
	try {
		...
		throw new Error(
			`Executor did not become reachable at ${instance.baseUrl} (port ${instance.port}) within ${config.startupTimeoutMs}ms. Check ${instance.logFile}.`,
		);
	}
...
export const getInstanceRecordPath = (instanceId: string, agentDir?: string): string =>
	join(getAgentDir(agentDir), INSTANCE_DIRECTORY_NAME, "instances", instanceId, INSTANCE_RECORD_FILE_NAME);

export const getConfigPaths = (cwd: string, agentDir?: string): { global: string; project: string } => ({
