...
import { createExecution, getExecution, getInstallation, isTerminalStatus, pollExecution, resumeExecution, } from "./executor-http.js";
...
const EXECUTE_TOOL_DESCRIPTION = "Run TypeScript code in executor. Use executor's discovery workflow: discover tools, inspect schemas, then call tools.*.";
...
const updateStatus = async (ctx, services) => {
    try {
        ...
        ctx.ui.setStatus(STATUS_KEY, restored?.lastSeenStatus === "waiting_for_interaction" ? STATUS_WAITING : STATUS_READY);
    }
    catch {
        ctx.ui.setStatus(STATUS_KEY, STATUS_ERROR);
    }
};
const openBrowserUrl = async (services, url) => {
    try {
...
const formatStatusMessage = (instance, reachable, scopeId) => reachable
...
const createRegisteredExtension = (services) => (pi) => {
    const persistState = (details) => {
        pi.appendEntry(SESSION_ENTRY_TYPE, createState(details));
    };
    const getRunningContext = async (cwd, signal) => {
    ...
    pi.registerCommand("executor-web", {
        ...
        handler: async (_args, ctx) => {
            ...
            ctx.ui.notify(openError
                ? `Executor is ready at ${instance.baseUrl}. Browser open failed: ${openError}`
                : `Executor is ready at ${instance.baseUrl}`, openError ? "warning" : "info");
        },
    });
    pi.registerCommand("executor-login", {
        ...
        handler: async (_args, ctx) => {
            ...
            ctx.ui.notify(openError
                ...
                : `Executor login opened at ${loginUrl}`, openError ? "warning" : "info");
        },
    });
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
            ...
            const created = await services.createExecution(instance.baseUrl, installation.scopeId, params.code, signal ? { signal } : undefined);
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params, signal, _onUpdate, ctx) => {
            const { instance, installation } = await getRunningContext(ctx.cwd, signal);
            const current = await services.getExecution(instance.baseUrl, installation.scopeId, params.executionId, signal ? { signal } : undefined);
            ...
            if (params.responseJson !== undefined) {
                const resumed = await services.resumeExecution(instance.baseUrl, installation.scopeId, params.executionId, params.responseJson, signal ? { signal } : undefined);
                const settled = await services.pollExecution(instance.baseUrl, installation.scopeId, params.executionId, {
                    ...(signal ? { signal } : {}),
...
export const createPiExecutorExtension = (overrides) => createRegisteredExtension({
    ...defaultServices,
    ...overrides,
});
