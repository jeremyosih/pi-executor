...
import { createExecution, getExecution, getInstallation, parseInteractionPayload, pollExecution, resumeExecution, } from "../src/executor-http.js";
const waitingEnvelope = () => ({
    execution: {
        id: "exec-1",
        scopeId: "workspace-1",
...
test("executor HTTP helpers reject malformed responses", async () => {
    const fetchImpl = async () => new Response(JSON.stringify({ scopeId: "workspace-1", actorScopeId: "actor-1", resolutionScopeIds: [123] }), {
        status: 200,
    });
...
test("executor HTTP helpers preserve non-JSON HTTP errors", async () => {
    const fetchImpl = async () => new Response("not found", { status: 404, statusText: "Not Found" });
    await assert.rejects(() => getInstallation("http://127.0.0.1:8788", { fetchImpl }), /404 Not Found: not found/);
});
