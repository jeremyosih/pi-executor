...
import { ensureExecutorRunning, resolveExecutorInstance, } from "../src/instance.js";
...
test("ensureExecutorRunning reports actionable timeout errors", async () => {
    const sandbox = await createSandbox();
    await assert.rejects(() => ensureExecutorRunning(sandbox.workspace, defaultConfig, {
        agentDir: sandbox.agentDir,
...
test("ensureExecutorRunning fails fast when configured port is occupied by another process", async () => {
    const sandbox = await createSandbox();
    await assert.rejects(() => ensureExecutorRunning(sandbox.workspace, { ...defaultConfig, port: 4801 }, {
        agentDir: sandbox.agentDir,
