...
test("loadConfig reads global config", async () => {
    const sandbox = await createSandbox();
    await writeFile(join(sandbox.agentDir, "pi-executor.json"), JSON.stringify({
...
test("project config overrides global config", async () => {
    ...
    await writeFile(join(sandbox.workspace, ".pi", "pi-executor.json"), JSON.stringify({
...
test("loadConfig reports malformed JSON", async () => {
    ...
    await assert.rejects(() => loadConfig(sandbox.workspace, { agentDir: sandbox.agentDir }), /malformed JSON/);
});
test("loadConfig validates field values", async () => {
    ...
    await assert.rejects(() => loadConfig(sandbox.workspace, { agentDir: sandbox.agentDir }), /"loginPath" must be a relative path|"port" must be between 1 and 65535/);
});
